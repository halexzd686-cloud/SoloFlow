...
def show(
    ...
    if agent.heartbeat.enabled:
        console.print(
            f"[bold]Heartbeat:[/bold] {agent.heartbeat.interval} — {agent.heartbeat.trigger_prompt[:80]}"  # noqa: E501
        )
...
heartbeat_app = typer.Typer(help="心跳定时调度", no_args_is_help=True)
...
def heartbeat_start(
    agent_name: str = typer.Argument(..., help="Agent 名称"),
    daemon: bool = typer.Option(False, "--daemon", "-d", help="作为后台守护进程运行"),
):
    """启动 Agent 心跳定时任务。

    --daemon 模式在后台独立进程中运行，关闭终端后继续执行。
    输出写入 .soloflow/heartbeats/<agent>.log。

    Examples:
        sf agent heartbeat start my-agent              # 前台运行
        sf agent heartbeat start my-agent --daemon     # 后台守护进程
    """
    try:
        agent = _load_agent(agent_name)
    except FileNotFoundError:
    ...
    if not agent.heartbeat.enabled:
        console.print(
            f"[yellow]Agent '{agent_name}' 未启用心跳。"
            f"请先编辑 Agent 定义，设置 heartbeat.enabled = true[/yellow]"
        )
        raise typer.Exit(1)

    import asyncio

    from soloflow.core.heartbeat import start_heartbeat
    ...
    if ok and not daemon:
        ...
        console.print("[dim]按 Ctrl+C 停止心跳[/dim]")
...
def heartbeat_stop(
    ...
    Examples:
        sf agent heartbeat stop my-agent
    """
    from soloflow.core.heartbeat import stop_heartbeat
...
def heartbeat_list():
    ...
    Examples:
        sf agent heartbeat list
    """
    from soloflow.core.heartbeat import show_heartbeat_status
...
def heartbeat_resume():
    ...
    扫描 .soloflow/heartbeats/ 目录，找出之前运行中但进程已死的心跳，
    以 daemon 模式重新启动。

    Examples:
        sf agent heartbeat resume
    """
    from soloflow.core.heartbeat import resume_heartbeats

    console.print("[dim]Scanning for interrupted heartbeats...[/dim]")
